...
public class ScFollowPlanFragment extends SherlockFragment{
    ...
    public void displayfollowPlanInfo() {
        ...
        if (bshowcustomerinfo) {
            followPlanInfo.add(new BasicInfoListAdapter.Info(infokeylist1[0],
                ...
                false, false));
            followPlanInfo.add(new BasicInfoListAdapter.Info(infokeylist1[1],
                BaseInfoRowViewItem.MOBILETEXT_TYPE, null,
                (customer != null && customer.getCustMobile() != null ? customer.getCustMobile()
                    : ""), false, false));
                        ...
                        new AlertDialog.Builder(getActivity())
                            ...
                            .setNegativeButton(getString(R.string.cancel),
                                new DialogInterface.OnClickListener() {
                                    ...
                                    @Override
                                    public void onClick(DialogInterface dialog, int which) {
                                        followPlanInfoAdapter.setEditable(followPlanInfoAdapter
                                            .getEditable());
                                        newFollowPlanInfoAdapter.setEditable(true);
                                        editData = !editData;
    ...
    public void dataModify(String key, String value, String pairKey) {
        ...
        if(getString(R.string.executestatus).equals(key)){
            if(getString(R.string.executestatus_2).equals(value)){
                for(BasicInfoListAdapter.Info info : followPlanInfo){
...
//        if(getString(R.string.executetime).equals(key)){
...
//                item = (BaseInfoRowViewItem)mFollowListView.findViewWithTag(getString(R.string.executetime));
//                item.setValue(String.valueOf(newTracePlan.getExecuteTime()));
//                for(BasicInfoListAdapter.Info info : newFollowPlanInfo){
//                    if(key.equals(info.key)){
//                        info.value = DateFormatUtils.formatDate(newTracePlan.getExecuteTime());
//                        break;
//                    }
//                }
//            }
...
//            if(Long.parseLong(value) < DateFormatUtils.systemDate()){
//                Toast.makeText(getActivity(), getString(R.string.execute_time_small), Toast.LENGTH_LONG).show();
//                item = (BaseInfoRowViewItem)mNewFollowListView.findViewWithTag(key);
//                item.setValue(String.valueOf(tracePlan.getExecuteTime()));
//                for(BasicInfoListAdapter.Info info : followPlanInfo){
//                    if(key.equals(info.key)){
//                        info.value = DateFormatUtils.formatDate(tracePlan.getExecuteTime());
//                        break;
//                    }
//                }
//            }
